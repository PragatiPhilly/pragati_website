...
beforeAll(async () => {
  ...
  CREATE TABLE IF NOT EXISTS members (id text PRIMARY KEY, user_id text NOT NULL, family_name text NOT NULL, primary_first_name text NOT NULL, primary_last_name text NOT NULL, phone text, address_line1 text, address_line2 text, city text, state text, zip text, country text DEFAULT 'US', membership_status text NOT NULL DEFAULT 'pending_payment', membership_started_at date, notes text, created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(), deleted_at timestamptz);
