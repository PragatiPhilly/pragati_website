...
function coverageLabel(dayKeys: unknown, days: EventDay[]): string {
  ...
  return keys
    .map((k) => days.find((d) => d.key === k)?.label.split(',')[0] ?? k)
    .join(' + ');
}
...
function fmtTime(hhmm: string | null): string | null {
  if (!hhmm) return null;
  return new Date(`2000-01-01T${hhmm}:00`).toLocaleTimeString('en-US', {
    hour: 'numeric',
    minute: '2-digit',
  });
}
...
function PriceCell({ t }: { t: Ticket }) {
  ...
  const save =
    !membersOnly && t.priceNonmemberCents > t.priceMemberCents
      ? t.priceNonmemberCents - t.priceMemberCents
      : 0;
    ...
    <div className="leading-tight whitespace-nowrap">
      {!membersOnly && (
        <div className="flex items-baseline gap-1.5">
          <span
            className="w-16 text-[10px] font-bold uppercase tracking-wider"
            style={{ color: 'var(--ink-soft)' }}
          >
            Guest
          </span>
          <span className="font-[family-name:var(--font-display)] font-bold">
            {formatCents(t.priceNonmemberCents)}
          </span>
        </div>
      )}
      <div className="flex items-baseline gap-1.5">
      ...
      {save > 0 && (
        <p
          className="text-[10px] font-semibold pl-[4.25rem]"
          style={{ color: 'var(--leaf-deep)' }}
        >
          save {formatCents(save)}
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="mt-6">
      <div className="flex items-baseline gap-2 mb-2.5">
        <h3 className="font-[family-name:var(--font-display)] text-lg font-black">
          {title}
        </h3>
        {sub && (
...
function CoverageCard({ label, passes }: { label: string; passes: Ticket[] }) {
  const sorted = [...passes].sort(
    (a, b) => Number(b.withFood) - Number(a.withFood),
  );
  return (
    <div
    ...
    >
      ...
      <div className="grid grid-cols-[1fr_auto_auto] gap-x-4 gap-y-1.5 items-baseline text-sm">
        ...
        {sorted.flatMap((t) => [
          ...
          <span
            key={`${t.id}-g`}
            className="font-[family-name:var(--font-display)] font-bold text-right"
          >
...
function SimpleRows({ passes }: { passes: Ticket[] }) {
  return (
    <div
      className="festive-card divide-y"
...
function AddonCard({ t, days }: { t: Ticket; days: EventDay[] }) {
  ...
  return (
    ...
    >
      <p className="font-bold text-sm">🎫 {t.name}</p>
      <p className="text-xs mt-0.5" style={{ color: 'var(--ink-soft)' }}>
        {keys.length > 0 && keys.length < days.length
          ? `${coverageLabel(t.dayKeys, days)} · `
          : ''}
        {time ? `${time} · ` : ''}
...
export default async function EventDetailPage({
  ...
  const youth = event.ticketTypes.filter(
    (t) => t.ageBand === 'child_5_18' || t.ageBand === 'child_5_12',
  );
    ...
    <div className="mx-auto max-w-5xl px-5 py-14">
      <Reveal>
        ...
        <h1 className="font-[family-name:var(--font-display)] text-4xl md:text-6xl font-black">
          ...
          {event.nameBengali && (
            <span
              className="font-[family-name:var(--font-bangla)] block text-2xl md:text-3xl mt-3 font-normal"
              style={{ color: 'var(--ink-soft)' }}
            >
              {event.nameBengali}
            </span>
        ...
        <Reveal delay={0.08}>
          <div className="mt-9 flex justify-center">
            <div
              ...
              style={{
                aspectRatio:
                  eventImage.height > eventImage.width ? '4 / 5' : '16 / 10',
        ...
        <div className="festive-card mt-8 p-6 flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          {event.venueMapUrl && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="btn-secondary !py-2 !px-5 text-sm w-fit"
            >
              Open in Maps
            </a>
          )}
      ...
      <Reveal delay={0.25}>
        <div className="mt-14">
          ...
          {studentGroups.length > 0 && (
            <TicketGroup
              title="Students"
              sub="🎓 valid student ID required at entry"
            >
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
